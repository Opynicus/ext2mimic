...
constexpr int INODE_BITMAP_START_ADDR = (BLOCK_BITMAP_START_ADDR + 20 * BLOCK_SIZE);    //inode位图 偏移地址，占2个磁盘块，最多监控 512 * 2 个inode的状态
constexpr int INODE_TABLE_START_ADDR (INODE_BITMAP_START_ADDR + 2 * BLOCK_SIZE);    //inode节点区 偏移地址，占 BLOCK_NUM_PER_INODE 个磁盘块
constexpr int DATA_BLOCK_START_ADDR = (INODE_TABLE_START_ADDR + BLOCK_NUM_PER_INODE * BLOCK_SIZE);//block数据区 偏移地址 ，占 INODE_NUM 个磁盘块constexpr int ROOT_DIR_ADDR = INODE_TABLE_START_ADDR;
